...
export default function MissingLetterGame({ game, onComplete, light }) {
  ...
  return (
    <div>
      ...
      <div className={`${light ? 'bg-white border-gray-200' : 'bg-white/10 border-white/20'} backdrop-blur-xl border rounded-3xl p-6 text-center`}>
        <h2 className="text-3xl font-bold mb-6 flex justify-center items-center flex-wrap text-gray-800">
          {displayWord}
        </h2>
        {!showResult && (
        ...
        {showResult && (
          <div className="mt-4 animate-slide-up">
            <div className={`text-xl font-bold ${message.includes('🎉') ? 'text-green-500' : 'text-red-500'}`}>{message}</div>
          </div>
        )}
      </div>
    </div>

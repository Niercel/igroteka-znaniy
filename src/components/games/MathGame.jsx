...
export default function MathGame({ game, onComplete, difficulty, onDifficultyChangeRequest, light }) {
  ...
  // Сброс всего при изменении сложности
  useEffect(() => {
    setRound(0)
    setScore(0)
    setStreak(0)
  ...
  // Генерация нового вопроса при смене раунда
  useEffect(() => {
    setQuestion(generateMathQuestion(settings.numberRange, settings.operators))
    setSelected(null)
    setShowResult(false)
      ...
      <div className={`${bgCard} backdrop-blur-xl border rounded-3xl p-6 text-center`}>
        ...
        {showResult && (
          <div className="mt-4 animate-slide-up">
            <div className={`text-lg font-bold mb-1 ${selected === question.answer ? 'text-green-500' : 'text-red-500'}`}>
              {message}
            </div>

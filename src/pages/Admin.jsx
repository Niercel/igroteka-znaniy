...
const CONTENT_TYPES = {
  word: {
    label: 'Слово (Читалочка)', fields: ['word', 'image', 'level', 'order'],
    placeholders: { word: 'кот', image: '🐱', level: '1', order: '1' },
    required: ['word', 'level'],
  },
  buildWord: {
    label: 'Слово для сборки', fields: ['word', 'image', 'level', 'order'],
    placeholders: { word: 'КОТ', image: '🐱', level: '1', order: '1' },
    required: ['word', 'level'],
  },
  sentence: {
    label: 'Предложение (Чтение)', fields: ['text', 'image', 'level', 'order'],
    placeholders: { text: 'Мама мыла раму', image: '🧼', level: '1', order: '1' },
    required: ['text', 'level'],
  },
  category: {
    label: 'Категория предметов', fields: ['categoryName', 'emojis', 'order'],
    placeholders: { categoryName: 'Фрукты', emojis: '🍎,🍊,🍋', order: '1' },
    required: ['categoryName', 'emojis'],
  },
  matchingPair: {
    label: 'Пара для сопоставления', fields: ['pair', 'connection', 'order'],
    placeholders: { pair: '🍎,🍊', connection: 'фрукты', order: '1' },
    required: ['pair', 'connection'],
  },
  missingLetter: {
    label: 'Пропущенная буква', fields: ['word', 'missingIndex', 'options', 'level', 'order'],
    placeholders: { word: 'кот', missingIndex: '1', options: 'о,а,у', level: '1', order: '1' },
    required: ['word', 'missingIndex', 'options', 'level'],
  },
  syllables: {
    label: 'Слоги', fields: ['syllables', 'word', 'level', 'order'],
    placeholders: { syllables: 'ко,т', word: 'кот', level: '1', order: '1' },
    required: ['syllables', 'word', 'level'],
  },
  chooseWord: {
    label: 'Выбор слова', fields: ['image', 'word', 'options', 'level', 'order'],
    placeholders: { image: '🐱', word: 'КОТ', options: 'КОТ,КИТ,РОТ,ДОМ', level: '1', order: '1' },
    required: ['image', 'word', 'options', 'level'],
  },
  describeImage: {
    label: 'Картинка для описания', fields: ['image', 'expected', 'level', 'order'],
    placeholders: { image: '🐱', expected: 'кот', level: '1', order: '1' },
    required: ['image', 'expected', 'level'],
  },
  sentenceWords: {
    label: 'Предложение (Составь)', fields: ['words', 'level', 'order'],
    placeholders: { words: 'Мама,моет,раму', level: '1', order: '1' },
